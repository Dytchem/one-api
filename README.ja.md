...
> このプロジェクトは [songquanpeng/one-api](https://github.com/songquanpeng/one-api) のフォークであり、元のプロジェクトのすべての機能を継承しています。

### 🔧 主な変更点

このフォークは **Fallback メカニズム** を改善しています：

#### 1. リトライ時のモデルマッピング損失を修正
**問題**: リクエストがチャンネル間で fallback 发生时、以前の試行で既にマッピングされていたモデル名が再利用され、その後のチャンネルが自分の `model_mapping` を適用できなくなります。

**修正** (`relay/controller/text.go`):
- リトライ時に `ctxkey.OriginalModel` から元のモデル名を復元
- 各チャンネルが自分の `model_mapping` を適用できるようにする

#### 2. Fallback ループロジックを書き直し
**問題**: 元のコードは「最後の」失敗チャンネルのみをスキップし、`CacheGetRandomSatisfiedChannel` の固定ランダムシードは同じ優先順位で常に同じチャンネルを返していました。

**修正** (`controller/relay.go`):
- `failedChannelIds` セットを維持し、すべての既知の失敗チャンネルを除外
- 新しい `GetRandomSatisfiedChannelExcluding` でDBを直接クエリして優先順位間の fallback を実現

#### 3. 優先順位間 Fallback をサポート
**問題**: ある優先順位のすべてのチャンネルが失敗した場合、システムはより低い優先順位のチャンネルに fallback できませんでした。

**修正** (`model/ability.go`):
- `GetRandomSatisfiedChannelExcluding` は最高優先順位から順に試行
- 各優先順位内でのランダム選択、現在レベルがすべて失敗した場合のみ次の優先順位に降格

#### 4. ストリーム応答 Fallback を最適化
**問題**: ストリーム応答モードでは、 空応答のチェック前にデータがクライアントに書き込まれ、fallback 後、新しいチャンネルの応答が前のデータと混信し、**クライアントが混乱する**可能性がありました。

**修正** (Stream Probe):
- ストリームリクエストはまず「プローブ」: ストリームリクエストを送信し、すべての応答データをキャッシュ
- キャッシュから使用量情報を抽出して有効なコンテンツがあるかどうかを確認 (`completion_tokens > 0`)
- `completion_tokens == 0` の場合、直ちに fallback をトリガー (**クライアントにはデータを送信しない**)
- プローブが成功した後、キャッシュされたデータをクライアントにリプレイ
- クライアントの体験: 完全な正常なストリーム応答、**ただ遅延がある**(プローブ時間)

### 🤖 開発支援

このプロジェクトでのすべてのコード変更、テスト、検証は [OpenClaw](https://github.com/openclaw/openclaw) AI Agent によって支援されました。要件分析、コード変更、Docker イメージビルド、データベースシミュレーションテストなどが含まれます。

### 📄 ライセンス

このプロジェクトは元のプロジェクトの MIT ライセンスを継承しています。

<details>
<summary><b>📋 変更概要</b></summary>

| ファイル | 変更内容 |
|---------|---------|
| `relay/controller/text.go` | リトライ時に元のモデルを復元 + Stream probe |
| `controller/relay.go` | failedChannelIds セットで fallback ループを書き直し |
| `model/ability.go` | `GetRandomSatisfiedChannelExcluding` 関数を追加 |
| `relay/adaptor/oneapi/relay/adaptor.go` | SSE ストリーミング用の SetEventStreamHeaders |
</details>
