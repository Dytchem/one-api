...
> 本项目为 [songquanpeng/one-api](https://github.com/songquanpeng/one-api) 的分支，继承了原项目的所有功能。
...
**前端修复**：
- ✅ Channel/Token/User/Logs/Redemptions 表格排序功能修复
- ✅ Channel/Token 等表格的分页大小 (itemsPerPage) 下拉框修复
- ✅ 修复 React 闭包问题，排序参数正确传递

**后端修复**：
- ✅ Channel API 支持 `order`、`sort`、`size` 参数
- ✅ Token API 支持 `size` 参数
- ✅ Relay 渠道尝试日志记录（便于调试）

**部署优化**：
- ✅ GitHub Actions 自动构建 Docker 镜像 (ghcr.io/dytchem/one-api)

#### Fallback 机制说明

以统一模型名 `jarvis` 请求为例，演示 fallback 机制的实际效果：

**模型映射配置**：

| 渠道 | 优先级 | 模型映射 |
|------|--------|---------|
| Channel A | 20 | jarvis → MiniMax-M2.7 |
| Channel B | 15 | jarvis → openai/gpt-4o |
| Channel C | 10 | jarvis → claude-3.5-sonnet |
| Channel D | 5 | jarvis → gemini-2.0-flash |
| Channel E | 0 | jarvis → qwen-plus |

**Fallback 执行过程**：

| # | 渠道 | 优先级 | 结果 | 耗时 |
|---|------|--------|------|------|
| 1 | Channel A | 20 | ❌ Token 失效 / 网络错误 | +2s |
| 2 | Channel B | 15 | ❌ 401 Unauthorized | +1s |
| 3 | Channel C | 10 | ❌ 认证失败 | +1s |
| 4 | Channel D | 5 | ❌ 请求超时 | +5s |
| 5 | Channel E | 0 | ✅ **成功** → qwen-plus | +2s |

**结果**：总耗时 11 秒，请求成功，模型映射 `jarvis → qwen-plus` 正确应用。
...
本分支对 OneAPI 的 **Fallback 机制** 进行了以下核心改进：

#### 1. 修复模型映射丢失问题
**问题**：请求在渠道间 fallback 时，已映射的模型名被复用，导致后续渠道无法正确应用自己的模型映射。

**修复**（`relay/controller/text.go`）：
- 重试时从 `ctxkey.OriginalModel` 恢复原始模型名
- 确保每个渠道都能应用自己的 `model_mapping`

#### 2. 重写 Fallback 循环逻辑
**问题**：原版只跳过"上一个"失败渠道，且随机数种子固定导致同优先级总是返回同一个渠道。

**修复**（`controller/relay.go`）：
- 维护 `failedChannelIds` 集合，排除所有已知失败渠道
- 新增 `GetRandomSatisfiedChannelExcluding` 直接查数据库

#### 3. 支持跨优先级 Fallback
**问题**：同优先级渠道全部失败后，无法降级到低优先级渠道。

**修复**（`model/ability.go`）：
- `GetRandomSatisfiedChannelExcluding` 从最高优先级向下尝试
- 每级内随机选择，全部失败才降级到下一优先级

#### 4. 流式请求 Fallback 优化
**问题**：流式响应中数据已写入客户端后才检测 empty response，fallback 后新渠道的响应会与之前的混合，**导致客户端收到混乱数据**。

**修复**：
- 流式请求先进行"探测"：发送流式请求并缓存所有响应数据
- 从缓存中提取 usage 信息检查是否有有效内容（`completion_tokens > 0`）
- 如果 `completion_tokens == 0`，立即触发 fallback（**不向客户端发送任何数据**）
- 探测成功后，将缓存的数据回放给客户端
- 客户端体验：完全正常的流式响应，**只是有延迟**（探测时间）

### 🤖 协助工具

本项目的修改、测试和验证工作全程由 [OpenClaw](https://github.com/openclaw/openclaw) AI Agent 协助完成，包括需求分析、代码修改、Docker 镜像构建、数据库模拟测试等。
...
1. 从 [GitHub Releases](https://github.com/songquanpeng/one-api/releases/latest) 下载可执行文件或者从源码编译：
   ```shell
   git clone https://github.com/Dytchem/one-api.git
...
- [OpenClaw](https://github.com/openclaw/openclaw) — 本项目修改由 OpenClaw AI Agent 协助完成
