...
以下是该机制在实际使用中的效果演示。请求使用统一模型名 `jarvis`，每个渠道配置自己的映射：

```
请求: jarvis
模型映射配置:
  Channel A     priority=10  jarvis → MiniMax-M2.7
  Channel B         priority=8   jarvis → minimaxai/minimax-m2.7  
  Channel C          priority=8   jarvis → minimax/MiniMax-M2.5
  Channel D          priority=0   jarvis → qwen3.5-flash

Fallback 过程:
  
  Channel 17 (p=10) → ❌ Token 失效 / 网络错误
         ↓
  Channel 13 (p=8)  → ❌ 401 Unauthorized
         ↓
  Channel 4  (p=8)  → ❌ 认证失败
         ↓
  Channel 2  (p=0)  → ✅ 成功! jarvis → qwen3.5-flash
```

**结果**: 请求成功返回 `qwen3.5-flash` 的回答，模型映射正确应用。

> [!NOTE]
> 如果所有渠道均失败，返回错误：`"无可用渠道"`。
...
#### 1. 修复模型映射丢失问题
**问题**：当请求在渠道间 fallback 时，已映射的模型名被复用，导致后续渠道无法正确应用自己的模型映射。

**修复**（`relay/controller/text.go`）：
- 在重试时从 `ctxkey.OriginalModel` 恢复原始模型名
- 确保每个渠道都能应用自己的 `model_mapping`

#### 2. 重写 Fallback 循环逻辑
**问题**：原版只跳过"上一个"失败渠道，且 `CacheGetRandomSatisfiedChannel` 的随机数种子固定导致同优先级总是返回同一个渠道。

**修复**（`controller/relay.go`）：
- 维护 `failedChannelIds` 集合，排除所有已知失败渠道
- 实现 `GetRandomSatisfiedChannelExcluding` 直接查数据库，支持跨优先级 fallback
